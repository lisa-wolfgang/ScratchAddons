export default async function ({ addon, global, console }) {
  ...
  if (addon.auth.isLoggedIn) {
    // If they are logged in, activate the rest of the code
    var username = addon.auth.username; // Get the username

    async function getfollowing(offset) {
      // Async function to get an array of following using the scratch api.
      return new Promise((resolve, reject) => {
        var returned = []; //What the function will return
        fetch("https://api.scratch.mit.edu/users/" + username + "/following?offset=" + offset + "&limit=40").then(
          async function (resp) {
            //Fetch 40 results from the scratch servers
    ...
    while (true) {
      ...
      if (following.includes(element.querySelector("a").innerText)) {
        //If we are following them...
